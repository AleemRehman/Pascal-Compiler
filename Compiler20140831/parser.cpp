...
namespace swd
{
	void Node::print(){ //print tree
		for (auto node : list){ //node for the list
			std::cout <<"{ " <<"type:" <<node->value.tag<<" | "
				<<"node"<<" }" << endl;
			node->print(); //print the node
		}
	}

	void Statement::print()
	{
		std::cout << "{ " << "type:" << this->value.tag /*tah value for the iter*/ << " | " << "Statements" << " }" << " ";  //statements print
		for (auto item : list){
			item->print(); //print for every item in the list
		}
	}
